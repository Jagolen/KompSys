...
def run_game():
    ...
    for q in q_list:
        ...
        for i in range(10):
            ...
            sharer_arr.append(state_count(rows, cols, current_generation, 1))
            q_arr.append(q)
    sharer_arr = [x/cols for x in sharer_arr]
    plt.hist2d(q_arr, sharer_arr, bins=10)
    plt.xlabel('p')
    plt.ylabel('Proportion of Sharers')
    plt.title(f'Heatpap of Sharers after 2000 generations using q = {q}, r = {r}')
    plt.show()

...
def run_game():
    ...
    resting = [0]*iterations
    sharing = [0]*iterations
    bored = [0]*iterations
    # Run A Firing Brain sequence
    resting[0] += state_count(rows, cols, current_generation, 0)
    sharing[0] += state_count(rows, cols, current_generation, 1)
    bored[0] += state_count(rows, cols, current_generation, 2)
    ...
    plt.plot(range(0,iterations), bored)
    plt.legend(['resting', 'sharing', 'bored'])
    plt.xlabel('Time')

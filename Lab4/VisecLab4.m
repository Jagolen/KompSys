...
eta=linspace(0,2*pi,nreta); %e is eta the noise parameter, whose maximum value is 2*pi
polarization = zeros(nreta,1);
...
%For all time steps
    ...
    for j=1:J
        ...
        for i=1:N
                ...
                T(i,j+1)=S+eta(e)*(rand-0.5); %adds noise to the measured angle
    ...
    polarization(e) = (1/N)*sqrt(sum(sin(T(:,end)))^2 + sum(cos(T(:,end)))^2);

end

plot(eta,polarization,'*')
xlabel("\eta")
ylabel("Polarisation")
title("Plot of how the polarisation changes depending on \eta")

...
def move_people(grid, gridval, N, M, people_pos, escape_time, dead_time, time_step, scared_factor, fire_factor, hm):
...
def main():
    ...
    if args.mode_mode == 'single':
        ...
        for rond in range(rounds):
            print_anything(f"{round(float(rond+1)/rounds*100,2)}%")
            # Grid of objects: 0 = empty, 1 = person, 2 = door, 3 = wall/furniture, 4 = fire
            grid = np.zeros([N,M])
            ...
            while True:
                ...
                gridval = next_gridval
                i+=1
                grid, left_in_room = move_people(grid, gridval, N, M, people_pos, escape_time, dead_time, i+1, scared_factor, fire_factor, hm)
                if len(escape_time) + len(dead_time) == num_people:
                    break
        grid = np.zeros([N,M])

...
def place_fire(grid, gridval, N, M, a, b):
    xlist = [a]
    ylist = [b]
    for x in xlist:
        for y in ylist:
...
def fire_spread(grid, gridval, N, M):
...
def move_people(grid, gridval, N, M, people_pos, escape_time, dead_time, time_step, scared_factor):

    fire_spread(grid,gridval,N,M)
    intended_move = []
    next_grid = np.zeros([N,M])
    for i in range(N):
        for j in range(M):
...
def main():
    ...
    if args.fire:
        a = 7
        b = 16
        place_fire(grid, gridval, N, M, a, b)
    ...
    if args.mode_mode == 'single':
        ...
        while True:
            ...
            xx.append(i)
            yy.append(len(escape_time))
            yy2.append(len(dead_time))
        ...
        plt.plot(xx,yy)
        plt.plot(xx,yy2)
        plt.legend(['Escaped', 'Died'])
        plt.show()
    
    elif args.mode_mode == 'scared_factor':
        scared_factor_list = [x/20 for x in range(0,19,1)]
        nr_means = 10
        for scared_factor in scared_factor_list:
            ...
            for k in range(nr_means):
                ...
                while True:
                    ...
                    if len(escape_time) + len(dead_time) == num_people:
                        break
                survived += len(escape_time)
                died += len(dead_time)
            surv = survived/nr_means
            dead = died/nr_means
                
            xx.append(scared_factor)        
            yy.append(surv)
            yy2.append(dead)
        ...
        plt.ylabel('People')
        plt.legend(['Survived, Fire: Back','Died, Fire: Back', 'Survived, Fire: Front', 'Died, Fire: Front'])
        plt.show()  
    ...
    elif args.mode_mode == 'fire_spread':
        global spread_rate
        fire_spread_rate = [x/100 for x in range(0,91,1)]
        nr_means = 20
        scared_factor = 0.05
        for spread_rate in fire_spread_rate:
            survived = 0
            died = 0
